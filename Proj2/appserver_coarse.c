...
pthread_mutex_t mut_lock_bank; // array of mutex locks for all accounts
...
int main(int argc, char **argv){              
	...
	pthread_mutex_init(&mut_lock_bank, NULL);
...
void *worker(void *arg){
	...
	if(list->head->check_acc_id > 0 && list->head->check_acc_id <= numAccounts){// its an balance check request
	...
	}else if(list->head->num_trans > 0){
		pthread_mutex_lock(&mut_lock_bank);		
		...
		pthread_mutex_unlock(&mut_lock_bank);
		//remove the request from the queue	
		list->num_jobs = list->num_jobs -1;
		list->head= list->head->next;
